...
/**
 * 对比HashMap来看 问题1 HashMap为什么不能支持并发？ 1.7版本的HashMap put数据的时候会成环
 * 1.8虽说不会成环形，但是有一定概率会覆盖数据 ps：HashMap扩容的是时候元素不会再一次hash 问题2
 * HashMap不是线程安全的那么有什么线程安全的Map类数据结构？HashTable、ConcurrentHashMap
 ...
 * 0;//（3）记录链表长度的变量，当binCount>8之后就会变成红黑树 for (Node<K,V>[] tab = table;;) {
 * Node<K,V> f; int n, i, fh; if (tab == null || (n = tab.length) == 0)
 * //（4）如果说数组为空的话或者数组长度==0，就初始化数组 tab = initTable();（5）初始化数组 else if ((f =
 * tabAt(tab, i = (n - 1) & hash)) == null)
 * {//（6）根据put的元素打的key的hash值来计算下标，并判断该位置是否已经有元素了，其中下标的算法和hashmap的一样
 * //都是数组大小-1按位与hash if (casTabAt(tab, i, null, new Node<K,V>(hash, key, value,
 * null))) （7）数组桶数据为空的话，使用cas替换，是线程安全的 break; // no lock when adding to empty
 * bin } else if ((fh = f.hash) == MOVED) tab = helpTransfer(tab, f);（8）扩容数组
 * else { V oldVal = null; synchronized (f) {（8）使用锁 if (tabAt(tab, i) == f) { if
 * (fh >= 0) { binCount = 1; for (Node<K,V> e = f;; ++binCount) { K ek; if
 * (e.hash == hash && ((ek = e.key) == key || (ek != null && key.equals(ek)))) {
 * oldVal = e.val; if (!onlyIfAbsent)（9）在链表上比较元素如果发现是一样的就替换 e.val = value;
 * break; } Node<K,V> pred = e; if ((e = e.next) == null) {（10）如果发现不一样就往下一个位置放元素
 * pred.next = new Node<K,V>(hash, key, value, null); break; } } } else if (f
 * instanceof TreeBin) { Node<K,V> p; binCount = 2; if ((p =
...
public class ConcurrentHashMapDemo {
    ...
    public static void main(String[] args) {
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                for (int i = 0; i < 1000; i++) {
                    concurrentHashMapDemo.put("key" + i, Thread.currentThread().getName() + i);
                }
            }
        });
        thread.start();
        ...
        for (int i = 0; i < 1000; i++) {
            System.out.println(concurrentHashMapDemo.get("key" + i));
        }
